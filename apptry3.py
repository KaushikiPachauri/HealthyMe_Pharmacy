...
# 🔹 NEW: Saved items model (for save-for-later feature)
...
# 🔹 Save item for later
@app.route("/save_item/<int:medicine_id>")
def save_item(medicine_id):
    if "user_id" not in session:
        return redirect(url_for("login"))
    cart_item = Cart.query.filter_by(user_id=session["user_id"], medicine_id=medicine_id).first()
    if cart_item:
        saved = SavedItem(user_id=session["user_id"], medicine_id=medicine_id)
...
@app.route("/move_to_cart/<int:medicine_id>")
def move_to_cart(medicine_id):
    if "user_id" not in session:
        return redirect(url_for("login"))
    saved_item = SavedItem.query.filter_by(user_id=session["user_id"], medicine_id=medicine_id).first()
    if saved_item:
        existing = Cart.query.filter_by(user_id=session["user_id"], medicine_id=medicine_id).first()
        if existing:
            existing.quantity += 1
        else:
            new_item = Cart(user_id=session["user_id"], medicine_id=medicine_id)
...
def cart():
    ...
    saved_items = SavedItem.query.filter_by(user_id=session["user_id"]).all()
    return render_template_string(CART_PAGE, cart=user_cart, total=total, saved_items=saved_items)
...
# 🔹 Updated Cart page with Save for Later section
CART_PAGE = BOOTSTRAP + NAVBAR + '''
<div class="container">
  <h3 class="text-success mb-3">Your Cart 🛒</h3>
  {% if cart %}
  <table class="table table-bordered bg-white shadow-sm">
    <thead><tr><th>Medicine</th><th>Qty</th><th>Price</th><th>Action</th></tr></thead>
    <tbody>
    {% for item in cart %}
      <tr>
        ...
        <td><a href="{{url_for('save_item', medicine_id=item.medicine.id)}}" class="btn btn-sm btn-outline-secondary">💾 Save for later</a></td>
      </tr>
    {% endfor %}
    </tbody>
  </table>
  ...
  <hr>
  <h4 class="text-success mt-4">Saved for Later 💾</h4>
  {% if saved_items %}
  <table class="table table-bordered bg-white shadow-sm">
    ...
    {% for s in saved_items %}
      <tr>
        ...
        <td>₹{{s.medicine.price}}</td>
        <td><a href="{{url_for('move_to_cart', medicine_id=s.medicine.id)}}" class="btn btn-sm btn-outline-success">🛒 Move to Cart</a></td>
      </tr>
    {% endfor %}
    </tbody>
  </table>
